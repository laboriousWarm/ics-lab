...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
	...
	int signA = (argA>>63) & 0x1;
	int signB = (argB>>63) & 0x1;
	int signV = (val>>63) & 0x1;

	if ((op==A_ADD) && ((!(signA^signB)) && (signA^signV))){
		ovf = TRUE;
	}

	if ((op==A_SUB) && ((signA^signB) && (signA^signV))){
		ovf = TRUE;
	}
...
stat_t nexti(y64sim_t *sim)
{
    ...
    if ((icode==I_RRMOVQ)||(icode==I_IRMOVQ)||(icode==I_RMMOVQ)
		...
		if (!get_byte_val(sim->m, next_pc, &codefun)){
			err_print("PC = 0x%lx, Invaild stack address 0x%lx", sim->pc, next_pc);
			return STAT_INS;	
		}
	...
	if ((icode==I_IRMOVQ)||(icode==I_RMMOVQ)||(icode==I_MRMOVQ)
		...
		if (!get_long_val(sim->m, next_pc, &valC)){
			err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, next_pc);
			return STAT_ADR;
		}
		next_pc += 8;
	}
    ...
    switch (icode)
	{
      	...
      	case I_RRMOVQ:  /* 2:x regA:regB */
		{
			...
			cond_t cond = ifun;
			bool_t doit = cond_doit(sim->cc, cond);
			if (doit)
			{
				long_t valE = compute_alu(A_ADD, 0, valA);
		...
		{
			long_t valE = compute_alu(A_ADD, valC, 0);
			set_reg_val(sim->r, regB, valE);
			sim->pc = next_pc;
			break;
		}
      	...
      	case I_RMMOVQ: /* 4:0 regA:regB imm */
		{
            ...
            long_t valB = get_reg_val(sim->r, regB);
 			long_t valE = compute_alu(A_ADD, valB, valC);
      	...
      	case I_ALU: /* 6:x regA:regB */
		{
		    long_t valA = get_reg_val(sim->r, regA);
			long_t valB = get_reg_val(sim->r, regB);	
			long_t valE = compute_alu(ifun, valA, valB);
      	...
      	case I_CALL: /* 8:x imm */
		{
			...
			long_t valE = compute_alu(A_SUB, 8, valB);
     	...
     	case I_RET: /* 9:0 */
		{
			...
			long_t valB = get_reg_val(sim->r, REG_RSP);
			long_t valE = compute_alu(A_ADD, 8, valB);
			set_reg_val(sim->r, REG_RSP, valE);
			long_t valM;
