...
{
	symbol_t *symptr = symtab->next;
	
	while (symptr != NULL)
	{
		if (strcmp(symptr->name, name) == 0)
			return symptr;
...
int add_symbol(char *name)
{
	...
	symbol_t *symnew;	
	symnew = (symbol_t *)malloc(sizeof(symbol_t));
...
void rel_val(reloc_t *rtmp, int begin, int len, long value)
{
	for (int i = 0; i < len; ++i)
    {
    	(rtmp->y64bin)->codes[begin+i] = HPACK((value>>(i*8+4))&0xF, (value>>(i*8))&0xF);
    }
}
