...
void Setpgid(pid_t pid, int pgig);
 ...
 * trace09.txt, trace10.txt
 * %num - jobid, num - processid
 */
void do_bgfg(char **argv) 
{
	pid_t pid;
	...
	else if (isdigit(id[0])){ /* is a pid is a digit number*/
		pid = atoi(id);
...
void sigchld_handler(int sig) 
{
	...
	while ((pid = waitpid(-1, &status, WNOHANG|WUNTRACED))>0){
		...
		if (WIFSIGNALED(status)){ /* process is terminated by signal */
...
void Setpgid(pid_t pid, int pgid){
