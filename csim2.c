...
bool_t access_cache(int tag, int setIndex, int blockOffset, int accessSize)
{
	int bestLine = 0;
	...
	{	
		...
		switch(access)
		{
			case INSTRUCTION_LOAD: 
				access_cache(tag, setIndex, blockOffset, accessSize);
				break;
			case DATA_LOAD: case DATA_STORE:
				access_cache(tag, setIndex, blockOffset, accessSize);
				break;
			case DATA_MODIFY: 
				if (access_cache(tag, setIndex, blockOffset, accessSize) == FALSE);
					access_cache(tag, setIndex, blockOffset, accessSize);
				break;
			default: break;
		}
