...
//			void *left = NEXT_BLKP(bp);
			...
			{
				remainder += GET_SIZE(HDRP(prev));
				PUT(HDRP(prev), PACK(remainder, 0));
				PUT(FTRP(prev), PACK(remainder, 0));
		//		replace_freeblk(next, left);
			...
			if (!GET_ALLOC(HDRP(prev)))
			{
				...
				if (csize + prev_size >= asize)
				{
					...
					{
						PUT(HDRP(prev), PACK((csize + prev_size), 1));
						PUT(FTRP(prev), PACK((csize + prev_size), 1));
						free_size -= prev_size;
						remove_freeblk(prev);
					}
					else {
						size_t remainder = csize + prev_size - asize;
						void *next = prev + remainder;
					...
					//	replace_freeblk(next, left);
						...
						PUT(HDRP(prev), PACK(remainder, 0));
						PUT(FTRP(prev), PACK(remainder, 0));
						free_size = free_size - prev_size + remainder;
					}
					return prev;
				}
			}
