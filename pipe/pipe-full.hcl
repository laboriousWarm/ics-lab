...
wordsig D_ifun 'if_id_curr->ifun'
...
word f_pc = [
	D_icode == IJXX && D_ifun == 6 &&
		E_icode == IOPQ && E_ifun == 2 && M_valE <= 0 : D_valP; 
	# Mispredicted branch.  Fetch at incremented PC
	M_icode == IJXX && !M_Cnd 
		&& !(M_ifun == 6 &&  W_icode == IOPQ && W_ifun == 2) : M_valA;
...
bool D_bubble =
	# Mispredicted branch
	((E_icode == IJXX && !e_Cnd) && !(E_ifun == 6 &&
     	M_icode == IOPQ && M_ifun == 2)) ||
	# Stalling at fetch while ret passes through pipeline
