...
long long gen_f_pc()
{
    return (((((((if_id_curr->icode) == (I_JMP)) & ((if_id_curr->ifun) == 6)
              ) & ((id_ex_curr->icode) == (I_ALU))) & ((id_ex_curr->ifun)
             == 2)) & ((ex_mem_curr->vale) <= 0)) ? (if_id_curr->valp) : (((
            (ex_mem_curr->icode) == (I_JMP)) & !(ex_mem_curr->takebranch))
         & !((((ex_mem_curr->ifun) == 6) & ((mem_wb_curr->icode) == (I_ALU))
            ) & ((mem_wb_curr->ifun) == 2))) ? (ex_mem_curr->vala) : (
...
long long gen_D_bubble()
{
    return (((((id_ex_curr->icode) == (I_JMP)) & !(ex_mem_next->takebranch)
          ) & !((((id_ex_curr->ifun) == 6) & ((ex_mem_curr->icode) == 
              (I_ALU))) & ((ex_mem_curr->ifun) == 2))) | (!((
